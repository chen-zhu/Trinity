...
from tyrell.enumerator import Enumerator, SmtEnumerator, RandomEnumerator, DesignatedEnumerator
...
class MorpheusGenerator(object):
    ...
    _sfn: Callable[[Any], bool]

    def __init__(self,
                 spec: S.TyrellSpec,
                 interpreter: Interpreter,
                 sfn: Callable[[Any], bool] = lambda x:True):
        self._interpreter = interpreter
        self._spec = spec
        self._sfn = sfn

    def generate(self, max_depth, example):
        tmp_enumerator = RandomEnumerator(self._spec, max_depth = max_depth)
        while True:
            try:
                ...
                if self._sfn(tmp_eval):
                    print("YES")
                    return (
                        tmp_prog, 
                        Example(input=example.input, output=[tmp_eval])
                    )
                else:
                    continue
            except Exception:
...
def main():
    ...
    # # test the generator
    # tmp_enum = RandomEnumerator(spec, max_depth=4)
    # tmp_morph = MorpheusInterpreter()
    # tmp_builder = D.Builder(spec)
    # # tmp_prog = tmp_builder._from_sexp(pldi17_sol.solutions["test"])
    # while True:
    #     tmp_prog = tmp_enum.next()
    #     print("PROG:{}".format(tmp_prog))
    #     tmp_example = Example(input=['input0'], output=None)
    #     try:
    #         tmp_eval = tmp_morph.eval(
    #             tmp_prog,
    #             tmp_example.input,
    #         )
    #         print("EVAL:")
    #         print(robjects.r(tmp_eval))
    #         break
    ...
    train_sample = generator.generate(
        max_depth=2,
        example=Example(input=['input0'], output=None),
    )
    print(train_sample)
    input("PAUSE")
    ...
    synthesizer = Synthesizer(
        ...
        # enumerator=DesignatedEnumerator(spec, pldi17_sol.solutions[1]),
        decider=TestDecider(
            interpreter=MorpheusInterpreter(),
            examples=[
