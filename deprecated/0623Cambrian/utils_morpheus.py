...
CAMB_LIST += ["<aSUB_{}>".format(i) for i in range(CAMB_NCOL)]
CAMB_LIST += ["<bSUB_{}>".format(i) for i in range(CAMB_NCOL)]
CAMB_LIST += ["<cSUB_{}>".format(i) for i in range(CAMB_NCOL)]
CAMB_LIST += ["<dSUB_{}>".format(i) for i in range(CAMB_NCOL)]
CAMB_LIST += ["<aSUB_DUP>","<bSUB_DUP>","<cSUB_DUP>","<dSUB_DUP>"]

CAMB_LIST += [
    "<COM/{},{}>".format(p[0],p[1]) 
    for p in product(
        ["aNEW","aDUP","aOUT"]+["aCOL_{}".format(i) for i in range(CAMB_NCOL)],
        repeat=2,
    )
]
CAMB_LIST += [
    "<COM/{},{}>".format(p[0],p[1]) 
    for p in product(
        ["bNEW","bDUP","bOUT"]+["bCOL_{}".format(i) for i in range(CAMB_NCOL)],
        repeat=2,
    )
]
CAMB_LIST += [
    "<COM/{},{}>".format(p[0],p[1]) 
    for p in product(
        ["cNEW","cDUP","cOUT"]+["cCOL_{}".format(i) for i in range(CAMB_NCOL)],
        repeat=2,
    )
]
CAMB_LIST += [
    "<COM/{},{}>".format(p[0],p[1]) 
    for p in product(
        ["dNEW","dDUP","dOUT"]+["dCOL_{}".format(i) for i in range(CAMB_NCOL)],
        repeat=2,
    )
]
...
def camb_get_x2x_value(p_val, p_dic, p_sepdic, p_prefix, no_com=False):
    ...
    if len(dfound)==0:
                ...
                if len(tmp_split)<=1:
                    ...
                    for i in range(len(tmp_split)):
                        try:
                            ...
                            continue
                    left_ind = camb_get_x2x_value(tmp_split[0], p_dic, p_prefix, no_com=True)
                    right_ind = camb_get_x2x_value(tmp_split[1], p_dic, p_prefix, no_com=True)
                    dtoken = "<COM/{},{}>".format(
        ...
        if is_SUB:
            ...
            if len(sub_found)==0:
                ...
                dtoken = "<{}SUB_{}>".format(p_prefix,sub_found[0])
                is_COM = False
            else:
                dtoken = "<{}SUB_DUP>".format(p_prefix)
                is_COM = False
...
def camb_get_features(p0_obj, p1_obj, verbose=False):
    ...
    for i in range(min(CAMB_NCOL, dc1)):
        cmap[0,i] = camb_get_x2x_value(ac1[i],cd_dic, cd_sepdic,"c")
    # col to cell mapping
    dmap = numpy.zeros((CAMB_NROW,CAMB_NCOL)) # 0 is <PAD>
    for i in range(min(CAMB_NROW,dr1)):
        for j in range(min(CAMB_NCOL,dc1)):
            dmap[i,j] = camb_get_x2x_value(np1_obj[i,j],cd_dic, cd_sepdic,"d")
