...
sketch_list = [
	("separate","gather","neg_select"),
	("spread","mutate","neg_select"),
	("neg_gather","separate","spread"),
	("gather","unite","spread"),
	("neg_gather","unite","spread"),
	("gather","group_by","summarise"),
	("neg_gather","group_by","summarise"),
]
...
for i in range(m_setting["ntotal"]):
	...
	d_sketch = random.choice(sketch_list)
	print("# Sampling {}...".format(d_sketch),end="")
	sys.stdout.flush()
	tmp_ps = m_generator.get_new_chain_program(
		fixed_depth=m_setting["fixed_depth"],
		fixed_sketch=d_sketch,
	)
